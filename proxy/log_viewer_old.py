...
.entry { margin-bottom: 12px; border: 1px solid var(--border); border-radius: 6px; overflow: hidden; }
...
.section-title { font-size: 12px; font-weight: 600; color: var(--text-muted);
  margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em; }
.json-block { background: var(--bg-tertiary); border-radius: 4px; padding: 10px;
...
.url-line { font-family: 'SF Mono', Consolas, monospace; font-size: 13px;
  color: var(--accent); padding: 6px 10px; background: var(--bg-tertiary);
...
function formatJson(obj) {
  try {
    ...
    return `<pre><code>${escapeHtml(String(obj))}</code></pre>`;
  }
}
